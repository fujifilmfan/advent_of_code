...
def find_parts(schematic):
    """Parts are numbers that have a symbol adjacent to them."""
    for line_num, line in enumerate(schematic):
        # get all groups of digits in each line, gives `0 [('467', 0, 3), ('114', 5, 8)]`
        matches = [(m.group(), m.start(), m.end()) for m in digit_regex.finditer(line)]
        print(line_num, matches)
        for match in matches:
            symbols = set()
            print("MATCH: ", match)
            num, start, end = match
            if line_num != 0:
                symbols.update(set(schematic[line_num - 1][start:end]) - NON_SYMBOLS)
                # print("TOP: ", top)
            if line_num != len(schematic) - 1:
                symbols.update(set(schematic[line_num + 1][start:end]) - NON_SYMBOLS)
                # print("BOTTOM: ", bottom)
            if start != 0:
                symbols.update({schematic[line_num][start - 1]} - NON_SYMBOLS)
                # print("LEFT: ", left)
            if end != len(line):
                symbols.update({schematic[line_num][end]} - NON_SYMBOLS)
                # print("RIGHT: ", right)
            if line_num != 0 and start != 0:
                symbols.update({schematic[line_num - 1][start - 1]} - NON_SYMBOLS)
                # print("TOP LEFT: ", top_left)
            if line_num != 0 and end != len(line):
                symbols.update({schematic[line_num - 1][end]} - NON_SYMBOLS)
                # print("TOP RIGHT: ", top_right)
            if line_num != len(schematic) - 1 and start != 0:
                symbols.update({schematic[line_num + 1][start - 1]} - NON_SYMBOLS)
                # print("BOTTOM LEFT: ", bottom_left)
            if line_num != len(schematic) - 1 and end != len(line):
                symbols.update({schematic[line_num + 1][end]} - NON_SYMBOLS)
...
# find_parts(day3_input)

# Part 2
asterisk_regex = re.compile(r"(\*)")
all_asterisks = defaultdict(list)


def find_asterisks(schematic):
    for line_num, line in enumerate(schematic):
        matches = [m.start() for m in asterisk_regex.finditer(line)]
        for match in matches:
            all_asterisks[f"{line_num}-{match}"] = []


def find_gears(schematic):
    find_asterisks(schematic)
    for line_num, line in enumerate(schematic):
        # get all groups of digits in each line, gives `[('467', 0, 3), ('114', 5, 8)]`
        numbers = [(m.group(), m.start(), m.end()) for m in digit_regex.finditer(line)]
        # print(line_num, numbers)
        for match in numbers:
            ...
            if line_num != 0:
                asterisks = [
                    m.start() + start
                    for m in asterisk_regex.finditer(schematic[line_num - 1][start:end])
                ]
                for asterisk in asterisks:
                    all_asterisks[f"{line_num - 1}-{asterisk}"].append(num)
                # print("TOP: ", top)
            if line_num != len(schematic) - 1:
                asterisks = [
                    m.start() + start
            ...
            if start != 0:
                asterisks = [
                    start - 1
                    for _ in asterisk_regex.finditer(schematic[line_num][start - 1])
                ]
                for asterisk in asterisks:
                    all_asterisks[f"{line_num}-{asterisk}"].append(num)
                # print("LEFT: ", left)
            if end < len(line) - 1:
                asterisks = [
                    end for _ in asterisk_regex.finditer(schematic[line_num][end])
                ]
                for asterisk in asterisks:
                    all_asterisks[f"{line_num}-{asterisk}"].append(num)
                # print("RIGHT: ", right)
            if line_num != 0 and start != 0:
                asterisks = [
                    start - 1
                    for _ in asterisk_regex.finditer(schematic[line_num - 1][start - 1])
                ]
                for asterisk in asterisks:
                    all_asterisks[f"{line_num - 1}-{asterisk}"].append(num)
                # print("TOP LEFT: ", top_left)
            if line_num != 0 and end < len(line) - 1:
                asterisks = [
                    end for _ in asterisk_regex.finditer(schematic[line_num - 1][end])
                ]
                for asterisk in asterisks:
                    all_asterisks[f"{line_num - 1}-{asterisk}"].append(num)
                # print("TOP RIGHT: ", top_right)
            if line_num != len(schematic) - 1 and start != 0:
                ...
                for asterisk in asterisks:
                    all_asterisks[f"{line_num + 1}-{asterisk}"].append(num)
                # print("BOTTOM LEFT: ", bottom_left)
            if line_num != len(schematic) - 1 and end < len(line) - 1:
                asterisks = [
                    end for _ in asterisk_regex.finditer(schematic[line_num + 1][end])
                ]
                for asterisk in asterisks:
    ...
    total = 0
    print(all_asterisks)
    for k, v in all_asterisks.items():
        if len(v) == 2:
            print(k, v)
